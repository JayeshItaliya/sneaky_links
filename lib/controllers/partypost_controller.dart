...
class PartyPostController extends GetxController {

  String search = "";
  var myList = List<PartyPostModel>.empty(growable: true).obs;
  var page = 1;
  var id = 1;
  ...
  void getTask(var page) {
    try {
      if (isoffline.value == false) {
        ...
        TaskProvider().fetchPartyPostList(
          page,id,
        ).then((resp) {
  ...
  void getMoreTask(var page) {
    try {
      ...
      TaskProvider().fetchPartyPostList(page,id).then((resp) {
        if (resp!.length > 0) {
